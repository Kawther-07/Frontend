...
class _NewWelcomePageState extends State<NewWelcomePage> {
    ...
    setState(() {
      resources = jsonResponse.map((resource) {
        return {
          ...
          'article': resource['article'] ?? '', // Ensure article is not null
        };
      }).toList();
    });
  } else {
  ...
  children: resources.map((resource) {
    String title = resource['title'] ?? ''; // Handle null title
    String description = resource['description'] ?? ''; // Handle null description
    String imageUrl = resource['imageUrl'] ?? ''; // Handle null imageUrl
    String article = resource['article'] ?? ''; // Handle null article
