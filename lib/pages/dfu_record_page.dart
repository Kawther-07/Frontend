  ...
  Future<void> fetchDfuRecord(int patientId, int medicalRecordId) async {
    try {
      ...
      final response = await http
          .get(Uri.parse('http://192.168.1.3:8000/api/dfu-record/$patientId/$medicalRecordId'))
...
Widget build(BuildContext context) {
  return Scaffold(
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 20),
                          Text(
                            'Prediction: ${dfuRecord!['prediction'] ?? 'Not specified'}',
    ...
    floatingActionButton: FloatingActionButton(
      onPressed: () {
        if (dfuRecord!['prediction'] == 'Abnormal') {
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text('Warning!'),
                content: Text('You need to consult with your doctor.'),
                actions: [
                  TextButton(
                    onPressed: () {
          ...
          showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                title: Text('Reminder'),
                content: Text('Take care of your feet to prevent DFU.'),
                actions: [
                  TextButton(
                    onPressed: () {
                      Navigator.pop(context);
